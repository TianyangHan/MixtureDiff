...
class StableDiffusionCanvasPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # cat everything for batch
        region_init = []  # (row, col)
        region_end = []  # (row, col)
        

        for region_index in len(text2image_regions):
            ## region information
            region_init.append((text2image_regions[region_index].row_init, text2image_regions[region_index].col_init))
            region_end.append((text2image_regions[region_index].row_end, text2image_regions[region_index].col_end))

            
            if region==0:
                text_embedding = text2image_regions[region_index].encoded_prompt.clone().unsqueeze(0)
            else:
                text_embedding = torch.cat(text_embedding, text2image_regions[region_index].encoded_prompt.clone().unsqueeze(0))
        ...
        encode_text_embedding = torch.cat([uncond_embeddings, text_embedding])
        ...
        latents_shape = (1, self.unet.in_channels, canvas_height // 8, canvas_width // 8)
        ...
        #     # For classifier free guidance, we nyin weieed to do two forward passes.
