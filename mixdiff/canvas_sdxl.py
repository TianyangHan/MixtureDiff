...
class Text2ImageRegion(DiffusionRegion):
    ...
    prompt_2: str = ""  # Text prompt guiding the diffuser in this region
    negative_prompt: str = ""  # Text prompt guiding the diffuser in this region
    negative_prompt_2: str = ""  # Text prompt guiding the diffuser in this region
...
class StableDiffusionXLCanvasPipeline(
    DiffusionPipeline,
    FromSingleFileMixin,
    StableDiffusionXLLoraLoaderMixin,
    TextualInversionLoaderMixin,
    ...
    def __call__(
        ...
        regions: List[DiffusionRegion]=None,
        prompt_embeds1: Optional[torch.FloatTensor] = None,
        negative_prompt_embeds1: Optional[torch.FloatTensor] = None,
        pooled_prompt_embeds1: Optional[torch.FloatTensor] = None,
        negative_pooled_prompt_embeds1: Optional[torch.FloatTensor] = None,
        **kwargs,
    ):
        r"""
        ...
        original_size = original_size or (canvas_height, canvas_width)
        target_size = target_size or (canvas_height, canvas_width)
       ...
       # Split diffusion regions by their kind
        ...
        for region in text2image_regions:
            ...
            (
                ...
                prompt_2=region.prompt_2,
                device=device,
                num_images_per_prompt=num_images_per_prompt,
                do_classifier_free_guidance=self.do_classifier_free_guidance,
                negative_prompt=region.negative_prompt,
                negative_prompt_2=region.negative_prompt_2,
            ...
            region.encoded_prompt = prompt_embeds.to(device)
            region.add_text_embeds = add_text_embeds.to(device)
            region.add_time_ids = add_time_ids.to(device).repeat(batch_size * num_images_per_prompt, 1)
        ...
        latents = self.prepare_latents(
            ...
            canvas_height,
            canvas_width,
            regions[0].encoded_prompt.dtype,
            device,
            generator,
            latents,
        )
        ...
        for region in all_eps_rerolls:
            if region.noise_eps > 0:
                region_noise = latents[:, :, region.latent_row_init:region.latent_row_end, region.latent_col_init:region.latent_col_end]
                ...
                latents[:, :, region.latent_row_init:region.latent_row_end, region.latent_col_init:region.latent_col_end] += eps_noise
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for region in text2image_regions:
                    ...
                    added_cond_kwargs = {"text_embeds": region.add_text_embeds, "time_ids":  region.add_time_ids}
